      ...
      const response = await fetch(
        "https://api.groq.com/openai/v1/chat/completions",
        {
          ...
          headers: {
            ...
            Authorization:
              "Bearer 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", // Replace with your valid Groq API key
          },
          body: JSON.stringify({
...
`,
      ...
      if (content) {
        ...
        const alternativeSolution = extractSolution(
          content,
          "Suboptimal Solution"
        );
      ...
      setSolutions({
        optimal: {
          ...
          explanation:
            "Failed to fetch solutions. Please check your API key and try again.",
        },
        alternative: {
          code: `// Error: ${error.message}`,
          explanation:
  ...
  const extractSolution = (content, solutionType) => {
    ...
    const solutionContent = content.slice(
      startIndex,
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center bg-zinc-800/40 rounded-lg p-4 border border-zinc-700/30">
          <p className="text-sm text-zinc-300">
            Loading {codingLanguage} solutions...
          </p>
        </div>
      ) : (
        <div className="space-y-2">
          ...
          <div className="bg-zinc-800/40 rounded-lg p-2 border border-zinc-700/30 max-h-32 overflow-auto">
            <pre className="text-xs text-cyan-300 font-mono whitespace-pre-wrap">
              {solutions[activeSolution].code ||
                `// No ${codingLanguage} code available.`}
            </pre>
          </div>
          ...
          <div className="bg-zinc-800/40 rounded-lg p-2 border border-zinc-700/30 max-h-24 overflow-auto">
            <div className="flex items-start">
              <svg
                className="w-4 h-4 text-purple-400 mt-0.5 mr-2 flex-shrink-0"
                viewBox="0 0 20 20"
                fill="currentColor"
              >
                <path
